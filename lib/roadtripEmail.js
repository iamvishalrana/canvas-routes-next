...
// Plain, personal-looking shell (matching the main Broadcasts tool) — not
// the dark-banded branded `shell()` above. A free-text update from Jerry
// should read like an email he typed, not an official launch announcement.
export function buildRouteBroadcastHtml({ firstName, routeName, message }) {
  return buildPlainEmailShell(`
        ${firstName ? `<p style="margin:0 0 0.75em;font-family:Arial,Helvetica,sans-serif;font-size:15px;line-height:1.7;color:#333;">Hi ${esc(firstName)},</p>` : ''}
        <p style="margin:0 0 0.75em;font-family:Arial,Helvetica,sans-serif;font-size:15px;line-height:1.7;color:#333;white-space:pre-wrap;">${esc((message || '').trim())}</p>
        <p style="margin:0.75em 0 0;font-family:Arial,Helvetica,sans-serif;font-size:12px;line-height:1.6;color:#999;">You're receiving this because you registered interest in ${esc(routeName)}.</p>`)
}
