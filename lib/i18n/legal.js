...
export const legalChrome = {
  en: { eyebrow: 'Legal', faq: 'FAQ', back: '← Back', lastUpdated: 'Last updated: August 2026' },
  fr: { eyebrow: 'Juridique', faq: 'FAQ', back: '← Retour', lastUpdated: 'Dernière mise à jour : août 2026' },
}
...
export const privacyT = {
  en: {
    ...
    sections: [
      ...
      {
        title: 'Private photo galleries',
        body: "If you attend a Canvas Routes event, we may share a private link to your event photos with you even if you are not a Canvas Routes member — using the name and email address already on file for you from your event registration. To access this link, you must confirm you own that email address by entering a one-time code we send you; that code, and a temporary access session once you're verified, are stored securely and expire automatically (see \"Who processes your data\" for details). We do not use this information for any purpose beyond giving you access to your own event photos.",
      },
      {
      ...
      },
      {
        ...
        body: "We use the following third-party services to operate Canvas Routes: Supabase (supabase.com) stores your personal information in a secure database, including membership applications, member profiles, and event registrations, and stores photos and media — including member profile and gallery photos, event photos, and event check-in car photos — in secure file storage. Stripe (stripe.com) processes payment card information for membership fees and event registrations. Stripe handles, stores, and secures all payment card data on our behalf — we do not store raw card numbers. Resend (resend.com) delivers confirmation and notification emails. Meta Pixel (meta.com) collects anonymised behavioural data on our website to support advertising and analytics through Facebook and Instagram — this data is subject to Meta's own privacy policy and advertising practices. Upstash (upstash.com) provides rate limiting by temporarily storing your IP address to prevent abuse — this data is not linked to your identity and expires automatically. For private photo galleries shared with non-members (see \"Private photo galleries\" above), Upstash also temporarily stores the one-time verification code sent to your email and, once you're verified, an access session linked to that email so you aren't asked to re-verify on return visits — this session expires automatically after 30 days and is only ever used to control access to that specific gallery. Vercel (vercel.com) hosts the website and processes all web requests — basic technical data including IP addresses may be processed for infrastructure and security purposes. Google Maps (google.com) powers the interactive route maps on our site — when a map loads, Google receives standard technical data such as your IP address, subject to Google's own privacy policy. Sentry (sentry.io) provides error monitoring — if a technical error occurs while you use the site, technical details of the request (which may include your IP address) are captured so we can fix the problem. Anthropic (anthropic.com) provides AI assistance used by our administrators to draft communications — limited application details (such as name and vehicle) may be processed for this purpose, and Anthropic does not use this data to train its models. All third-party services are engaged solely to operate Canvas Routes and are contractually restricted from using your personal data for purposes beyond what we have authorised.",
      },
      {
  ...
  fr: {
    ...
    sections: [
      ...
      {
        title: 'Galeries de photos privées',
        body: "Si vous assistez à un événement Canvas Routes, nous pouvons vous partager un lien privé vers vos photos de l'événement même si vous n'êtes pas membre de Canvas Routes — en utilisant le nom et l'adresse courriel déjà au dossier à partir de votre inscription à l'événement. Pour accéder à ce lien, vous devez confirmer que vous êtes propriétaire de cette adresse courriel en saisissant un code à usage unique que nous vous envoyons; ce code, et une session d'accès temporaire une fois vérifiée, sont conservés de manière sécurisée et expirent automatiquement (voir « Qui traite vos données » pour plus de détails). Nous n'utilisons pas ces renseignements à d'autres fins que de vous donner accès à vos propres photos d'événement.",
      },
      {
      ...
      },
      {
        ...
        body: "Nous utilisons les services tiers suivants pour exploiter Canvas Routes : Supabase (supabase.com) conserve vos renseignements personnels dans une base de données sécurisée, y compris les demandes d'adhésion, les profils de membres et les inscriptions aux événements, et conserve les photos et médias — y compris les photos de profil et de galerie des membres, les photos d'événements et les photos de véhicule prises à l'enregistrement — dans un espace de stockage de fichiers sécurisé. Stripe (stripe.com) traite les renseignements de carte de paiement pour les frais d'adhésion et les inscriptions aux événements. Stripe gère, conserve et sécurise toutes les données de carte de paiement en notre nom — nous ne conservons pas les numéros de carte bruts. Resend (resend.com) achemine les courriels de confirmation et de notification. Meta Pixel (meta.com) recueille des données comportementales anonymisées sur notre site web afin de soutenir la publicité et l'analyse par l'entremise de Facebook et Instagram — ces données sont assujetties à la propre politique de confidentialité et aux pratiques publicitaires de Meta. Upstash (upstash.com) assure la limitation du débit en conservant temporairement votre adresse IP afin de prévenir les abus — ces données ne sont pas liées à votre identité et expirent automatiquement. Pour les galeries de photos privées partagées avec des non-membres (voir « Galeries de photos privées » ci-dessus), Upstash conserve également temporairement le code de vérification à usage unique envoyé à votre courriel et, une fois vérifiée, une session d'accès liée à ce courriel afin que vous n'ayez pas à vérifier de nouveau lors de visites ultérieures — cette session expire automatiquement après 30 jours et ne sert qu'à contrôler l'accès à cette galerie précise. Vercel (vercel.com) héberge le site web et traite toutes les requêtes web — des données techniques de base, y compris les adresses IP, peuvent être traitées à des fins d'infrastructure et de sécurité. Google Maps (google.com) alimente les cartes interactives des routes sur notre site — lorsqu'une carte se charge, Google reçoit des données techniques standard telles que votre adresse IP, assujetties à la propre politique de confidentialité de Google. Sentry (sentry.io) assure la surveillance des erreurs — si une erreur technique survient pendant votre utilisation du site, les détails techniques de la requête (qui peuvent inclure votre adresse IP) sont capturés afin que nous puissions résoudre le problème. Anthropic (anthropic.com) fournit une assistance par IA utilisée par nos administrateurs pour rédiger des communications — des détails limités de la demande (comme le nom et le véhicule) peuvent être traités à cette fin, et Anthropic n'utilise pas ces données pour entraîner ses modèles. Tous les services tiers sont engagés uniquement pour exploiter Canvas Routes et sont contractuellement tenus de ne pas utiliser vos données personnelles à des fins autres que celles que nous avons autorisées.",
      },
      {
  ...
  en: {
    ...
    sections: [
      ...
      },
      {
        ...
        body: 'Photos, videos, and other content produced by Canvas Routes may not be reproduced, shared commercially, or used without prior written permission. By attending any Canvas Routes event — including meets and gatherings such as Cars & Coffee that are open to non-members — all attendees consent to being photographed or filmed for community use, including on the Canvas Routes website and social media channels.',
      },
      {
  ...
  fr: {
    ...
    sections: [
      ...
      },
      {
        ...
        body: 'Les photos, vidéos et autre contenu produits par Canvas Routes ne peuvent être reproduits, partagés commercialement ou utilisés sans autorisation écrite préalable. En assistant à tout événement Canvas Routes — y compris les rencontres et rassemblements comme Cars & Coffee ouverts aux non-membres — tous les participants consentent à être photographiés ou filmés à des fins communautaires, y compris sur le site web et les réseaux sociaux de Canvas Routes.',
      },
      {
