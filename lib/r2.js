...
// Direct server-side byte upload — used for migrating pre-existing objects
// from Supabase Storage into R2, bucket by bucket (photo-shares, receipts,
// and whichever of event-photos/member-photos/route-car-photos come next —
// the presigned-URL path above is for browser uploads only).
export async function putObject({ bucket, path, buffer, contentType }) {
  if (!r2) throw new Error('R2 not configured')
  const key = fullKey(bucket, path)
  await r2.send(new PutObjectCommand({ Bucket: R2_BUCKET, Key: key, Body: buffer, ContentType: contentType || 'application/octet-stream' }))
}
...
// Size + content-type without downloading the object — used to re-verify a
// migrated copy is intact (byte-size match against the source) before it's
// safe to delete the source elsewhere. Returns null if the object is missing
// rather than throwing, since "not there" is an expected, checkable outcome
// for this caller (never delete a source whose R2 copy can't be confirmed).
