...
const COPY = {
  fr: {
    eyebrow: 'Canvas Routes · Reçu',
    greeting: name => `Merci, ${name}.`,
    subtitle: 'Voici votre reçu — conservez-le pour vos dossiers.',
    receiptLabel: 'Reçu',
    billedTo: 'Facturé à',
    ...
    questions: 'Des questions à propos de ce paiement? Répondez directement à ce courriel ou écrivez-nous à info@canvasroutes.com.',
    footer: '© 2026 Événements Canvas Routes Inc. — Montréal, QC.',
    dateLocale: 'fr-CA',
  },
  en: {
    eyebrow: 'Canvas Routes · Receipt',
    greeting: name => `Thanks, ${name}.`,
    subtitle: "Here's your receipt — keep it for your records.",
    ...
    questions: 'Questions about this charge? Reply directly to this email or reach out at info@canvasroutes.com.',
    footer: '© 2026 Canvas Routes Events Inc. — Montreal, QC.',
    dateLocale: 'en-CA',
  },
}

function drawReceiptPage(doc, lang, data) {
  const t = COPY[lang]
  const { firstName, billedToName, billedToEmail, itemLabel, subtotal, discount, gst, qst, total, paidAt, receiptId } = data
  ...
  const dateStr = paidAt ? new Date(paidAt).toLocaleDateString(t.dateLocale, { year: 'numeric', month: 'long', day: 'numeric', timeZone: MONTREAL_TZ }) : ''
  ...
  doc.setFontSize(13)
  doc.setTextColor(26, 26, 26)
  doc.text(t.totalLabel, cardX + 4, afterTableY)
  doc.setTextColor(59, 107, 47)
  doc.text(`${fmt(total)} CAD`, cardX + cardW - 4, afterTableY, { align: 'right' })
  doc.setFont('helvetica', 'normal')
...
// Two-page bilingual PDF — page 1 French, page 2 English, identical content
// and styling on both, satisfying the Charter's "at least as favourable"
// prominence requirement without doubling every line inside a single page.
export function buildReceiptPdfBuffer(data) {
  const doc = new jsPDF({ unit: 'mm', format: 'a4' })
  drawReceiptPage(doc, 'fr', data)
  doc.addPage()
  drawReceiptPage(doc, 'en', data)
