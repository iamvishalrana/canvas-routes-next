...
export function buildBirthdayEmailHtml({ firstName, isMember }) {
  const body = isMember
    ...
    : `
      ${eyebrow('From the whole club')}
      ${p(`Happy birthday, ${firstName}! 🎉 Hope you have a great day today, whatever you end up doing with it.`)}
      ${p(`Wishing you good food, good company, and a good year ahead.`)}
      ${p('— Jerry & the Canvas Routes team', { tone: 'muted', mb: '0' })}
    `
  return emailShell({
...
export function birthdayEmailText({ firstName, isMember, unsubUrl }) {
  const message = isMember
    ...
    : `Happy birthday, ${firstName}! 🎉 Hope you have a great day today, whatever you end up doing with it.\n\nWishing you good food, good company, and a good year ahead.`
