...
// Exported so other ephemeral-Redis-backed features (e.g. lib/otp.js) reuse
// this exact client construction instead of re-deriving the env var names —
// a second, drifted copy is how you end up silently NOT sharing state across
// serverless instances in production while looking fine in local dev.
...
// `scope` namespaces the counter per action (e.g. 'gallery-request-code').
// Left blank (the default), every caller shares one `rl:${ip}` bucket — which
// is how every pre-existing call site still behaves, unchanged. Without a
// scope, a visitor on a shared/NAT'd IP (office wifi, mobile carrier CGNAT)
// who recently hit a *different* rate-limited endpoint can get a false 429
// on this one, and two routes on the same flow that intend separate budgets
// (e.g. gallery request-code vs. verify-code) actually share one counter —
// pass a scope for any route where that distinction matters.
export async function checkRateLimit(ip, maxRequests = MAX_REQUESTS, windowSec = WINDOW_SEC, scope = '') {
  const windowMs = windowSec * 1000
  const cacheKey = scope ? `${scope}:${ip}` : ip
  const key = `rl:${cacheKey}`
  const redis = getRedis()
  ...
  if (!entry || now > entry.resetAt) {
    localCache.set(cacheKey, { count: 1, resetAt: now + windowMs })
    return false
  }
