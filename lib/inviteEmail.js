...
export function buildInviteHtml(firstName, eventName, eventDate, eventLocation, rsvpUrl, expiresAt, isRoadTrip, isResend = false) {
  ...
  const body = isResend
    ? (isRoadTrip
        ? `Just a quick reminder &mdash; your invitation to join us for <strong style="color:#1a1a1a;font-weight:500;">${eventName}</strong> is still open. Confirm your spot below before the link expires.`
    ...
    : (isRoadTrip
        ? `We&rsquo;ve reviewed your application and we&rsquo;d love to have you on the road with us for <strong style="color:#1a1a1a;font-weight:500;">${eventName}</strong>. Confirm your spot below &mdash; we&rsquo;ll follow up with full route details as the date gets closer.`
      ...
      <tr><td style="background:#ffffff;padding:36px 40px 32px;">
        ...
        <table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin-bottom:28px;">
          <tr><td style="background:#0F1E14;">
            <a href="${rsvpUrl}" style="display:inline-block;padding:15px 36px;font-family:Arial,Helvetica,sans-serif;font-size:10px;letter-spacing:2.5px;text-transform:uppercase;color:#F5F1EC;text-decoration:none;font-weight:600;">Confirm My Spot &rarr;</a>
          </td></tr>
        </table>
