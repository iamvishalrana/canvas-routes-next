...
// Manually-added registrants (`registrations[].event`) store whatever name is
// on the actual `events` table row, which is admin-typed and can drift from
// the hardcoded WTET_EVENT_NAME above (different dash character, wording,
// spacing). Match fuzzily instead of by exact equality, or a manually-added
// registrant can silently fail every WTET lookup/save. Mirrors the fallback
// already used in app/admin/events/EventsClient.jsx.
export function isWtetEventName(name) {
  if (!name) return false
  if (name === WTET_EVENT_NAME) return true
  return name.toLowerCase().includes('eastern townships')
}
...
// Auberge et Restaurant McGowan, Georgeville, QC — curated lunch menu for July 5, 2026
export const WTET_LUNCH_OPTIONS = [
  ...
  { id: 'fish_and_chips', name: 'Fish and chips (poisson pané), frites et salade', description: 'Beer-battered fish, fries and salad' },
...
- A premium lunch at Auberge et Restaurant McGowan in Georgeville, QC, overlooking Lac Memphremagog
...
- Un déjeuner haut de gamme à l'Auberge et Restaurant McGowan à Georgeville, QC, avec vue sur le lac Memphrémagog
