...
import { nowInMontreal } from './mtlTime.js'
...
// A member's primary car's MAKE only (e.g. "BMW") — `cars` (JSONB array,
// member-only) wins if present, falling back to the flat car_make column
// every application (and older member rows) still use. Same source/fallback
// order as app/admin/members/MembersClient.jsx so "their car" means the same
// car everywhere it's shown, just trimmed to make only for the birthday email.
function primaryCarMake(p) {
  const make = (Array.isArray(p.cars) && p.cars.length > 0) ? p.cars[0]?.make : p.car_make
  return make?.trim() || null
}

export async function getBirthdays(supabase) {
  const [{ data: members }, { data: applications }] = await Promise.all([
    supabase.from('members').select('name, email, dob_month, dob_day, car_make, cars').not('dob_month', 'is', null).not('dob_day', 'is', null).not('email', 'is', null),
    supabase.from('applications').select('name, email, dob_month, dob_day, car_make').not('dob_month', 'is', null).not('dob_day', 'is', null).not('email', 'is', null),
  ])
  ...
  // Montreal-local "today", not server-UTC — Vercel functions run in UTC, so
  // a plain `new Date()` rolls over to the next calendar day ~7-8pm Montreal
  // time (DST-dependent), which would show the wrong day's birthday as
  // "today" (🎂 in AdminShell's sidebar widget) and throw daysUntil off by
  // one for that whole evening window. Same MONTREAL_TZ convention used
  // everywhere else in the codebase (lib/mtlTime.js).
  const { year: mtlYear, month: mtlMonth, day: mtlDay } = nowInMontreal()
  const today = new Date(mtlYear, mtlMonth - 1, mtlDay)
  const birthdays = []
