...
// Time-of-day display for events whose `date_display`/`date` fields (both
// date-only) don't carry a time. Keyed by event id (not name — names get
// edited) so both /meet/[id] and the members-portal event detail page show
// the same time without depending on an admin-typed free-text field. Add an
// entry per event that needs one; most events have no time to show at all.
export const EVENT_TIME_OVERRIDES = {
  '1a020f09-f618-42ed-b646-75c1927da38a': '9:00 AM – 11:30 AM', // Cars & Coffee — September 5, 2026
}
