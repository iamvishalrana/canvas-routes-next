...
export const EVENT_NAME_ALIASES = {
  ...
  // "Cars & Coffee" is a GENERIC name reused across multiple distinct meets
  // (there's a separate "Cars & Coffee" event on May 9), so it must NOT use the
  // "Name — Date" format: the registrant matcher strips "— <date>" to group
  // year-variants of the same route, which would collapse this Sept 5 meet with
  // every other "Cars & Coffee" event and show all ~120 of them as one. Map
  // both the em-dash name AND the old glued name to a PARENTHETICAL canonical
  // whose base ("Cars & Coffee (September 5, 2026)") is unique, keeping this
  // event's registrants correctly separate.
  'Cars & Coffee — September 5, 2026':                 'Cars & Coffee (September 5, 2026)',
