...
export async function buildCalendarFeed(supabase, { events: wantEvents, birthdays: wantBirthdays, notes: wantNotes, calName }) {
  ...
  for (const b of birthdays) {
    ...
    // getBirthdays already checks day/month are each in range (1-31 /
    // 1-12), but not that they form a real date — a stored DOB of e.g. Feb
    // 30 (bad manual entry, no day-count-per-month check upstream) would
    // otherwise emit an invalid DTSTART;VALUE=DATE literal. JS Date rolls an
    // impossible date silently forward instead of throwing, so catch it by
    // round-tripping and comparing rather than trusting the raw values.
    const check = new Date(`${anchor}T00:00:00Z`)
    if (check.getUTCMonth() + 1 !== b.month || check.getUTCDate() !== b.day) continue
    lines.push(
      'BEGIN:VEVENT',
