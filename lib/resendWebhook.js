...
export function verifyResendWebhook(rawBody, headers, secret) {
  ...
  // digest() with no encoding returns the raw 32-byte buffer directly — do
  // NOT round-trip through digest('base64') + Buffer.from(str), which
  // interprets the base64 string as UTF-8 text (44 bytes) instead of
  // decoding it back to the original 32 bytes, guaranteeing a length
  // mismatch against the provided signature (also 32 bytes) on every call.
