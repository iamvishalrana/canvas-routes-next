...
// check-in (trip details / waiver / lunch / car photo) yet — same visual
// template as buildWtetConfirmHtml's "Complete Early Check-in" button,
// framed as a nudge instead of a payment confirmation. Lists exactly which
// sections are still missing for THIS registrant (not a generic "trip
// details, waiver, and/or lunch, depending what's open") and includes the
// event date/location so the email stands on its own without requiring
// them to go dig up their original confirmation email.
export function buildCheckinReminderHtml({ firstName, checkinUrl, eventLabel, dateDisplay, location, missingLabels }) {
  ...
  const checklist = (missingLabels || []).map(label => `
    <tr><td style="padding:9px 0;border-bottom:0.5px solid rgba(0,0,0,0.06);">
      <table role="presentation" cellpadding="0" cellspacing="0" border="0"><tr>
        <td style="width:18px;vertical-align:top;padding-top:1px;">
          <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="14" height="14" style="border:1.5px solid #c5a882;"><tr><td>&nbsp;</td></tr></table>
        </td>
        <td style="font-family:Arial,Helvetica,sans-serif;font-size:13px;color:#333;padding-left:10px;">${label}</td>
      </tr></table>
    </td></tr>`).join('')
  ...
  <tr><td align="center" style="padding:32px 16px 48px;">
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="max-width:580px;">
      ...
      <tr><td style="background:#ffffff;padding:36px 40px 32px;">
        <p style="margin:0 0 20px;font-family:Georgia,'Times New Roman',serif;font-size:15px;line-height:1.85;color:#444;">You&apos;re registered for <strong style="color:#1a1a1a;font-weight:500;">${eventLabel}</strong>, but we still need the following from you before the day:</p>
