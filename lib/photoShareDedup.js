...
// to storage.
//
// findExistingSharedPhoto (the check) and this insert (the act) are two
// separate requests, so two admins — or one admin in two tabs — uploading
// the same byte-identical file to same-titled folders at the same moment
// can both pass the check before either has inserted. The DB's unique index
// on (content_hash, folder_title) stops the second insert from creating a
// duplicate canonical row, but by then the loser's browser has already
// uploaded both its files to storage. Without this recovery, that upload
// fails outright and the two now-unreferenced files leak in the bucket
// forever — cleanupOrphanedPhotos only ever looks at photo_ids it already
// knows about, so it can never find them. Recovering here instead: treat
// the constraint hit as proof the winner's row now exists, delete our own
// now-redundant storage files, and link into the winner's photo like any
// other duplicate.
export async function createSharedPhotoAndLink(admin, { folderId, storagePath, originalPath, contentHash, folderTitle, caption }) {
  ...
  if (photoErr) {
    if (photoErr.code === '23505' && contentHash) {
      const winner = await findExistingSharedPhoto(admin, { contentHash, folderTitle })
      if (winner) {
        const losingPaths = [...new Set([storagePath, originalPath].filter(Boolean))]
        if (losingPaths.length) removeObjects({ bucket: BUCKET, paths: losingPaths }).catch(() => {})
        const link = await linkExistingPhoto(admin, { folderId, photoId: winner.id, caption })
        return { photo: winner, link }
      }
    }
    throw photoErr
  }
...
export async function cleanupOrphanedPhotos(admin, photoIds) {
  ...
  for (const photo of (orphanPhotos || [])) {
    ...
    try {
      if (paths.length) {
        await removeObjects({ bucket: BUCKET, paths })
      }
