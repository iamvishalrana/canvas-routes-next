...
import { copyObject, getPublicUrl, removeObjects } from './r2'
...
export async function claimSharedPhotosForMember(admin, { memberId, email }) {
  ...
  for (const it of items) {
    ...
    if (!copyConfirmed) {
      try {
        // Both buckets are on R2 now — a native same-provider copy, no bytes
        // routed through this function. (Used to be a cross-provider
        // download-then-upload back when gallery-photos was still on
        // Supabase Storage; see git history if that ever needs resurrecting.)
        await Promise.all([
          copyObject({ bucket: SHARES_BUCKET, fromPath: it.storage_path, toBucket: GALLERY_BUCKET, toPath: dispDest }),
          copyObject({ bucket: SHARES_BUCKET, fromPath: origSrc, toBucket: GALLERY_BUCKET, toPath: origDest }),
        ])
        const photoUrl = getPublicUrl({ bucket: GALLERY_BUCKET, path: dispDest })
        const originalUrl = getPublicUrl({ bucket: GALLERY_BUCKET, path: origDest })
        ...
        if (insErr) {
          ...
          removeObjects({ bucket: GALLERY_BUCKET, paths: [dispDest, origDest] }).catch(() => {})
