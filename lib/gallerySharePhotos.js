...
export async function loadPersonFolders(admin, person) {
  ...
  for (const i of (links || [])) {
    ...
    const url = getPublicUrl({ bucket: BUCKET, path: i.photo.storage_path })
    const originalUrl = getPublicUrl({ bucket: BUCKET, path: i.photo.original_path || i.photo.storage_path })
