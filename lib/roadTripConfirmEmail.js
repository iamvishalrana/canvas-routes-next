...
// (WTET, Hello to Montebello, Calabogie Boogie, future routes) via
// eventLabel. Built on the shared email design system (lib/emailLayout.js).
import { emailShell, p, button, infoCard, instagram, defaultFooter, COLOR } from './emailLayout.js'

// Default eventLabel — only used by callers that omit it (none currently do).
const EVENT_NAME = 'Whips to Eastern Townships — July 5, 2026'

export function buildRoadTripHoldHtml(firstName, amount, eventLabel = EVENT_NAME) {
  const body = `
...
export function buildRoadTripConfirmHtml(firstName, amount, checkinUrl, eventLabel = EVENT_NAME) {
  const body = `
