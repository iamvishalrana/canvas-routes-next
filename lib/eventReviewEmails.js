...
import { emailShell, p, infoCard, accentCard, eyebrow } from './emailLayout.js'

export function buildPendingReviewHtml({ firstName, eventName, dateDisplay, location, photoUrl, timeDisplay }) {
  const body = `
...
export function buildAcceptedHtml({ firstName, eventName, dateDisplay, location, photoUrl, timeDisplay, calendarHtml = '' }) {
  const body = `
    ...
    ${infoCard([
      ...
      location && ['Location', location],
      ['Entry', 'Free'],
    ], { mb: calendarHtml ? '20px' : '28px' })}
    ${calendarHtml}
    ${accentCard(`
      ${eyebrow('Bring a Friend', { mb: '8px' })}
      ${p(`Know any car friends who&rsquo;d love this too? Tell them to register and mention your name &mdash; a referral from someone we already know is an easy yes.`, { tone: 'muted', mb: '0' })}`)}
    ${p(`Questions? Reply directly to this email &mdash; it comes straight to me.`, { mb: '6px' })}
    ${p(`&mdash; Jerry`, { tone: 'muted', mb: '0' })}
  `
  return emailShell({
...
export function buildDeclinedHtml({ firstName, eventName, photoUrl }) {
  const body = `
