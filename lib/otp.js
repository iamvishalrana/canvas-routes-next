...
export async function createSession(token, email, ttlSec = SESSION_TTL_SEC) {
  const id = randomBytes(32).toString('base64url')
  const key = sessionKey(id)
  const entry = { token, email, expiresAt: now() + ttlSec * 1000 }
  const redis = getRedis()
  if (redis) {
    try { await redis.set(key, JSON.stringify(entry), { ex: ttlSec }); return id } catch { /* fall through */ }
  }
