...
//
// This backs every public check-in route (lookup/trip-details/waiver/lunch/
// car-photo) — the public check-in URL is the same for every registrant,
// gated only by typing an email, so this MUST reject anyone whose payment
// never completed (declined, refunded, or simply abandoned mid-form) the
// same way listEventRegistrants() below already does for the admin dashboard.
// Without this, once any one legitimate registrant forwards the check-in
// link, anyone who ever started the form — paid or not — could complete
// trip-details/waiver/lunch/car-photo under their own email.
export async function findEventRegistrant(admin, eventId, eventName, email) {
  const [{ data: memberReg }, { data: app }] = await Promise.all([
    ...
    admin.from('applications')
      .select('id, name, email, registrations, car_year, car_make, car_model, stripe_payment_status')
      .eq('email', email)
      .maybeSingle(),
  ])
  ...
  if (app && matched) {
    if (!matched.paid && FAILED_PAYMENT_STATUSES.includes(app.stripe_payment_status)) return null
...
export async function listEventRegistrants(admin, eventId, eventName) {
  ...
  for (const app of apps || []) {
    ...
    // A declined review-flow registration (app/api/public/events/[id]/register)
    // must never count as a registrant — otherwise it holds a capacity slot
    // forever even though the admin explicitly turned this person away.
    // review_status only exists on that flow's entries; undefined here for
    // every other flow (paid road trips, member free-register) is a no-op.
    if (matchedReg.review_status === 'declined') continue
    ...
    byEmail.set(email, {
      ...
      lang: app.lang || null,
      // Convoy group / group lead — set from the admin Registrants panel,
      // lives on the per-event registrations[] entry (road-trip registrants
      // only; the member-portal event_registrations path above has no
      // group support).
      convoy_group: matchedReg.convoy_group ?? null,
      group_lead: !!matchedReg.group_lead,
...
// who shouldn't be asked — they've already submitted one (for this event or
// any other; it's the same car) or they already have a profile photo on
// their member record. Every caller that decides what to show/require/nag a
// registrant about for this section must go through this, not raw
// event.checkin_sections, or exempt people get asked/reminded forever since
// they'll never complete it.
//
// Deliberately does NOT skip based on "attended a route before" alone —
// WTET's bespoke check-in (app/wtet/checkin) never collected a car photo at
// all, digital or otherwise, so a WTET-only alumnus with no member profile
// photo has no photo anywhere in the system despite `attended: true` on
// their WTET registrations[] entry. Skipping them here would mean they're
// never asked and this system never has a photo of their car — worse than
// occasionally asking a repeat attendee a second time.
export async function resolveCheckinSectionsBatch(admin, emails, checkinSections) {
  ...
  const [{ data: priorPhotos }, { data: members }] = await Promise.all([
    admin.from('event_checkins').select('email').in('email', normEmails).not('car_photo', 'is', null),
    admin.from('members').select('email, cars, car_photo_url').in('email', normEmails),
  ])
  const alreadySent = new Set((priorPhotos || []).map(r => (r.email || '').toLowerCase()))
  const hasProfilePhoto = new Set(
    (members || [])
      .filter(m => m.cars?.[0]?.photo_url || m.car_photo_url)
      .map(m => (m.email || '').toLowerCase())
  )
  const map = new Map()
  for (const email of normEmails) {
    const skip = alreadySent.has(email) || hasProfilePhoto.has(email)
...
export async function listEventCandidates(admin, eventId, eventName) {
  ...
  return registrants.map(r => {
    ...
    // listEventRegistrants above) — this must come FIRST, ahead of the
    // member's general garage/profile car. It's the most specific source
    // (what they're actually bringing to THIS event, including an admin's
    // deliberate edit via the Registrants panel) — falling back to the
    // member's profile car only when no event-specific value was ever set.
    const year = r.registration?.carYear || primaryCar?.year || m?.car_year || a?.car_year || ''
    const make = r.registration?.carMake || primaryCar?.make || m?.car_make || a?.car_make || ''
    const model = r.registration?.carModel || primaryCar?.model || m?.car_model || a?.car_model || ''
    const photo = checkinByEmail.get(r.email)?.car_photo?.url || primaryCar?.photo_url || m?.car_photo_url || null
    return { name: r.name, email: r.email, car: formatCarLabel(year, make, model) || null, photo, group: r.convoy_group ?? null, lead: !!r.group_lead, paymentStatus: r.paymentStatus }
  })
}
