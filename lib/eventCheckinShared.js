...
export async function listEventRegistrants(admin, eventId, eventName) {
  const [{ data: memberRegs }, { data: apps }] = await Promise.all([
    admin.from('event_registrations')
      ...
      .in('stripe_payment_status', ['paid', 'free', 'authorized']),
    admin.from('applications')
      .select('id, name, email, registrations, stripe_payment_status, stripe_payment_intent_id, stripe_amount_paid, phone, instagram, passengers, has_children, children_ages, source, more, dob, car_year, car_make, car_model'),
  ])
  ...
  for (const r of memberRegs || []) {
    ...
    if (!email) continue
    byEmail.set(email, { name: r.members?.name || r.name || '', email, paymentStatus: r.stripe_payment_status || null, applicationId: null })
  }
...
export async function listEventCandidates(admin, eventId, eventName) {
  ...
  return registrants.map(r => {
    ...
    return { name: r.name, email: r.email, car: [year, make, model].filter(Boolean).join(' ') || null, photo }
  })
}
