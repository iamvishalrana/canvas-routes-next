...
**25. Rule 3's `'pending'` write only applies to the PI-creation route — any route that runs AFTER `confirmPayment()` must derive status from Stripe, never hardcode**
Real incident, 2026-07 (Elie Massuda application): `membership-waitlist` hardcoded `stripe_payment_status: 'pending'` on every call, copying rule 3's pattern — but unlike `wtet-register`/`hello-to-montebello-register` (which write `'pending'` *before* the PI exists, when nothing else could be true yet), `membership-waitlist` runs *after* `stripe.confirmPayment()` resolves, both in the normal flow and again from a 3DS-redirect-return handler (Instagram/Facebook in-app browsers routinely force a real page redirect for 3DS, not just an in-page challenge — see the `useEffect` in `components/MembershipContent.jsx` around the `redirect_status`/`payment_intent` query params). By the time either of those calls fires, the PI may already be `requires_capture`, and the webhook's `requires_capture` handler may have already raced ahead and written the correct `'authorized'` status. A hardcoded `'pending'` write at that point silently downgrades it back, and since the admin capture UI only renders for status `'authorized'`, the application becomes invisible to the normal admin workflow even though the hold is live and correct in Stripe.

Any route that retrieves/verifies a PaymentIntent from Stripe for any reason (which `membership-waitlist` already did, just wasn't using `pi.status` for this) must derive `stripe_payment_status` from that authoritative `pi.status` (`requires_capture` → `'authorized'`, `succeeded` → `'paid'`) instead of hardcoding a value — Stripe's own status for a given PI never regresses, so deriving from it is inherently race-proof regardless of which caller (webhook vs. client-triggered route) runs first. Only fall back to a hardcoded `'pending'` when no PI is being verified in that same request at all (the true PI-creation-time case rule 3 describes).
...
- **Timezone has no environment-level default, server or client.** `TZ` is a reserved variable name on Vercel (their functions run on AWS Lambda, which manages it internally) — confirmed by trying to set it 2026-09-01, rejected outright. There is no way to make the server's `Date` defaults automatically Montreal, so server and client code are in the same position: always be explicit. For any code (server or client) formatting a real timestamp (`created_at`, `signed_at`, `sent_at`, etc.), use `formatMtlDate`/`formatMtlTime`/`formatMtlDateTime` from `lib/mtlTime.js` instead of raw `toLocaleDateString`/`toLocaleString`/`toLocaleTimeString` — they force `timeZone: MONTREAL_TZ` unconditionally. For date-only values (no time-of-day, e.g. an event's `date` field) use `montrealTodayStr`/`montrealTodayDate` for "is this today or later" comparisons instead of `new Date(dateOnly)` (which parses as UTC midnight and misfiles "today" events as past for anyone west of UTC) — see `lib/mtlTime.js`'s own header comment for the full picture, this file is the single source of truth and was audited sitewide 2026-09-01 after this exact bug class recurred in 10+ files.
