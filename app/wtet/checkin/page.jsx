...
export default function WtetCheckinPage() {
  ...
  const [loadState, setLoadState] = useState('loading') // 'loading' | 'not_found' | 'already_done' | 'form'
  ...
  useEffect(() => {
    ...
    fetch(`/api/wtet-checkin?t=${encodeURIComponent(token)}`)
      ...
      .then(data => {
        ...
        if (data.alreadyCompleted) setLoadState('already_done')
        else setLoadState('form')
      })
  ...
  async function handleSubmit(e) {
    ...
    try {
      const res = await fetch('/api/wtet-checkin', {
        ...
        body: JSON.stringify({ token, dietary: dietary.trim() || null, whatsapp: whatsapp.trim() || null }),
      })
      if (!res.ok) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F5F1EC', fontFamily: 'var(--font-inter), Inter, sans-serif' }}>
      ...
      <main style={{ maxWidth: '520px', margin: '0 auto', padding: '7rem 1.5rem 6rem' }}>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              ...
              <div>
                ...
                <input
                  ...
                  style={{
                    width: '100%', padding: '0.75rem 0.9rem', border: '1px solid rgba(0,0,0,0.14)', background: '#fff',
                    fontSize: '14px', fontFamily: 'var(--font-inter), sans-serif', color: '#1a1a1a',
              ...
              <div style={{ padding: '1rem 1.1rem', background: 'rgba(197,168,130,0.07)', border: '0.5px solid rgba(197,168,130,0.3)' }}>
                ...
                <p style={{ fontSize: '13px', color: '#666', lineHeight: '1.75', margin: 0 }}>
                  ...
                  <a href="sms:+1-jerry" style={{ color: '#7B5B2E', textDecoration: 'none', fontWeight: '500' }}>jerry@canvasroutes.com</a>{' '}
