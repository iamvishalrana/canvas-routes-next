...
export default function LinksClient() {
  ...
  const [form, setForm] = useState({ name: '', url: '', password: '', eventDate: '', notes: '' })

  useEffect(() => {
    fetch('/api/admin/links').then(r => r.json()).then(d => { setLinks(Array.isArray(d) ? d : []); setLoading(false) }).catch(() => setLoading(false))
  }, [])

  async function handleAdd(e) {
    e.preventDefault()
    setSaving(true); setError('')
    const res = await fetch('/api/admin/links', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) })
    const data = await res.json()
    if (!res.ok) { setError(data.error || 'Failed to save.'); setSaving(false); return }
    setLinks(prev => [...prev, data])
    setForm({ name: '', url: '', password: '', eventDate: '', notes: '' })
    setAdding(false); setSaving(false)
  }

  async function toggleArchive(link) {
    const res = await fetch('/api/admin/links', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: link.id, archived: !link.archived }) })
    if (res.ok) setLinks(prev => prev.map(l => l.id === link.id ? { ...l, archived: !l.archived } : l))
  }

  async function handleDelete(id) {
    if (!confirm('Delete this link permanently?')) return
    const res = await fetch('/api/admin/links', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) })
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '860px', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
        ...
        <button onClick={() => setAdding(v => !v)} style={BTN(adding ? '#6b7280' : '#0F1E14')}>
        ...
        <form onSubmit={handleAdd} style={{ ...CARD, marginBottom: '2rem', background: '#f9fafb' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              <label style={{ fontSize: '11px', color: '#6b7280', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Event Name *</label>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              <label style={{ fontSize: '11px', color: '#6b7280', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Event Date</label>
              <input type="date" style={INPUT} value={form.eventDate} onChange={e => setForm(f => ({ ...f, eventDate: e.target.value }))} />
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', gridColumn: '1 / -1' }}>
              <label style={{ fontSize: '11px', color: '#6b7280', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.05em' }}>URL *</label>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              <label style={{ fontSize: '11px', color: '#6b7280', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Password</label>
              <input style={INPUT} placeholder="eastern" value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))} />
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              <label style={{ fontSize: '11px', color: '#6b7280', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Notes</label>
          ...
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            <button type="submit" disabled={saving} style={{ ...BTN('#0F1E14'), opacity: saving ? 0.6 : 1 }}>{saving ? 'Saving…' : 'Save Link'}</button>
          </div>
        </form>
      )}
      ...
      {loading ? (
        ...
        <>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: archived.length ? '2rem' : 0 }}>
            {active.sort((a, b) => (b.eventDate || '').localeCompare(a.eventDate || '')).map(link => (
              <LinkCard key={link.id} link={link} onArchive={() => toggleArchive(link)} onDelete={() => handleDelete(link.id)} />
            ))}
          </div>
...
function LinkCard({ link, onArchive, onDelete, isArchived }) {
  const formattedDate = link.eventDate ? new Date(link.eventDate + 'T12:00:00').toLocaleDateString('en-CA', { year: 'numeric', month: 'long', day: 'numeric' }) : null
  const isPast = link.eventDate && new Date(link.eventDate) < new Date()

  return (
    <div style={{ ...CARD, borderLeft: isArchived ? '3px solid #e5e7eb' : isPast ? '3px solid #f59e0b' : '3px solid #3B6B2F', position: 'relative' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem', flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', flexWrap: 'wrap', marginBottom: '0.25rem' }}>
            <span style={{ fontSize: '14px', fontWeight: '600', color: '#111' }}>{link.name}</span>
            {isPast && !isArchived && <span style={{ fontSize: '10px', background: '#fef3c7', color: '#92400e', padding: '2px 8px', borderRadius: '99px', fontWeight: '600', letterSpacing: '0.04em' }}>EVENT PASSED</span>}
            {isArchived && <span style={{ fontSize: '10px', background: '#f3f4f6', color: '#6b7280', padding: '2px 8px', borderRadius: '99px', fontWeight: '600', letterSpacing: '0.04em' }}>ARCHIVED</span>}
          </div>
          {formattedDate && <p style={{ fontSize: '12px', color: '#6b7280', margin: '0 0 0.6rem' }}>{formattedDate}</p>}
        </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
          <span style={{ fontSize: '11px', color: '#6b7280', fontWeight: '600', width: '64px', flexShrink: 0 }}>URL</span>
        ...
        {link.password && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
            <span style={{ fontSize: '11px', color: '#6b7280', fontWeight: '600', width: '64px', flexShrink: 0 }}>Password</span>
        ...
        {link.notes && (
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.5rem' }}>
            <span style={{ fontSize: '11px', color: '#6b7280', fontWeight: '600', width: '64px', flexShrink: 0, paddingTop: '1px' }}>Notes</span>
