...
import { Err, ToggleSwitch } from './shared'
...
export default function AwardsTallyClient({ eventId }) {
  ...
  const load = useCallback(() => {
    setLoading(true)
    fetch(`/api/admin/awards/${eventId}`)
  ...
  async function resetVotes() {
    ...
    setResetError(null)
    try {
      const res = await fetch(`/api/admin/awards/${eventId}/reset`, { method: 'POST' })
      const d = await res.json().catch(() => ({}))
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem' }}>
          <div>
            ...
            <div style={{ fontSize: '12px', color: '#888', marginTop: '0.2rem' }}>
              {data.event.awards_enabled ? 'Participants can submit and change their ballot.' : 'The public page shows "voting isn’t open yet."'}
            </div>
          </div>
          <ToggleSwitch checked={!!data.event.awards_enabled} onChange={toggleVoting} disabled={toggling} label="Voting open" />
        </div>
