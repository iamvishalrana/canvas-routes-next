...
function printWaiver(eventName, name, email, waiver) {
...
<h1>${esc(eventName)} — Signed Liability Waiver</h1>
<div class="meta">${esc(name)} &middot; ${esc(email)} &middot; Signed ${signedAt}</div>
<div class="waiver-body">${esc(waiver.waiver_text_snapshot)}${waiver.waiver_text_snapshot_fr ? `<br/><br/>${esc(waiver.waiver_text_snapshot_fr)}` : ''}</div>
<div class="sig-block">
...
// Same layout as WTET's WaiverViewerModal, generalized: reads the waiver's
// own waiver_text_snapshot/_fr (captured at signing time) instead of a
// hardcoded per-route text constant, and always shows both languages since
// the generic waiver form collects both together, not one-or-the-other.
export default function GenericWaiverViewerModal({ eventName, name, email, waiver, onClose }) {
  const vehicle = [waiver.vehicle?.year, waiver.vehicle?.make, waiver.vehicle?.model].filter(Boolean).join(' ') || '—'
  const signedAt = new Date(waiver.signed_at).toLocaleString('en-CA', { year: 'numeric', month: 'long', day: 'numeric', hour: 'numeric', minute: '2-digit', timeZone: MONTREAL_TZ })
      ...
      >
        <div style={{ padding: '1.25rem 1.5rem', borderBottom: '0.5px solid rgba(0,0,0,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, background: '#fff', zIndex: 1 }}>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
            <button onClick={() => printWaiver(eventName, name, email, waiver)} style={{ background: 'none', border: '0.5px solid rgba(0,0,0,0.18)', borderRadius: '8px', padding: '6px 12px', fontSize: '10px', letterSpacing: '0.1em', textTransform: 'uppercase', color: '#555', cursor: 'pointer', fontFamily: 'var(--font-inter),sans-serif' }}>
              Print / Save PDF
            </button>
        ...
        <div style={{ padding: '1.5rem' }}>
          <div style={{ marginBottom: '1.25rem', padding: '1rem 1.1rem', background: 'rgba(59,107,47,0.04)', border: '0.5px solid rgba(59,107,47,0.2)', borderRadius: '10px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '0.75rem', fontSize: '12px', color: '#444', fontFamily: 'var(--font-inter),sans-serif' }}>
              <div><div style={{ fontSize: '9px', letterSpacing: '0.1em', textTransform: 'uppercase', color: '#888' }}>Signed</div>{signedAt}</div>
              <div><div style={{ fontSize: '9px', letterSpacing: '0.1em', textTransform: 'uppercase', color: '#888' }}>IP Address</div>{waiver.ip_address}</div>
              <div><div style={{ fontSize: '9px', letterSpacing: '0.1em', textTransform: 'uppercase', color: '#888' }}>Vehicle</div>{vehicle}</div>
              <div><div style={{ fontSize: '9px', letterSpacing: '0.1em', textTransform: 'uppercase', color: '#888' }}>Emergency Contact</div><span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.1rem' }}>{waiver.emergency_contact?.name} · {waiver.emergency_contact?.phone}<CopyBtn value={waiver.emergency_contact?.phone} /></span></div>
            </div>
            {waiver.passengers?.length > 0 && (
              <div style={{ marginTop: '0.75rem' }}>
                <div style={{ fontSize: '9px', letterSpacing: '0.1em', textTransform: 'uppercase', color: '#888', marginBottom: '0.3rem', fontFamily: 'var(--font-inter),sans-serif' }}>Passengers</div>
