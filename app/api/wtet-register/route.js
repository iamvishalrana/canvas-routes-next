  ...
  try {
    ...
    if (process.env.RESEND_API_KEY && !_health_check) {
      after(() =>
        ...
        }).then(r => { if (r && !r.ok) captureMessage(`Resend non-200 — wtet-register-admin-notify`, { status: r.status }) }).catch(err => captureException(err, { context: 'wtet-register-admin-notify', email: normalEmail }))
      )
    }
