...
export async function GET() {
  try {
    ...
    const [{ data: member }, { data: application }, { data: eventRegs }] = await Promise.all([
      admin.from('members').select('id, name, email, phone, instagram, car_year, car_make, car_model, cars, dob_month, dob_day, dob_year, membership_status, tier, join_date, created_at, membership_number, car_photo_url, password_set_at, event_attendance').eq('id', user.id).maybeSingle(),
      normalEmail
