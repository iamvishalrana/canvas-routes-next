...
export async function POST(request) {
  ...
  const [{ data: ev }, { data: member }] = await Promise.all([
    admin.from('events').select('id, name, date, registration_opens_at, registration_closes_at, capacity, member_price, priority_window_end, registration_enabled').eq('id', eventId).single(),
