    ...
    switch (event.type) {
      ...
      case 'payment_intent.succeeded': {
        ...
        if (type === 'event_registration' && event_id && member_id) {
          ...
          // Durable per-event proof — see lib/markRegistrationPaid.js. Runs
          // unconditionally (member or not, admin-captured or webhook-driven)
          ...
          if (!isMember && !adminPanelAlreadyCaptured && process.env.RESEND_API_KEY && normalEmail) {
            // WTET has its own frozen check-in page; every other route
          ...
          after(() => Promise.allSettled([
            fetch('https://api.resend.com/emails', {
              ...
              body: JSON.stringify({
                ...
                // Same shared template the normal membership-waitlist flow
                // sends — so the applicant gets the identical email whichever
                // path wins the race (`type` maps to tier inside the builder).
                html: buildMembershipConfirmHtml(firstName, type),
