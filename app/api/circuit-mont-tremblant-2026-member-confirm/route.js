...
// Called by the Circuit Mont-Tremblant page immediately after stripe.confirmPayment()
...
export async function POST(request) {
  ...
  try {
    ...
    captureException(err, { context: 'cmt-member-confirm-retrieve', piId: paymentIntentId })
    return Response.json({ error: 'Could not verify payment' }, { status: 500 })
  }

  // Guard: must be a succeeded member Circuit Mont-Tremblant payment belonging to this user
  if (pi.status !== 'succeeded') return Response.json({ error: 'Payment not yet confirmed' }, { status: 400 })
  if (pi.metadata?.type !== 'road_trip_circuit-mont-tremblant-2026') return Response.json({ error: 'Invalid payment type' }, { status: 400 })
  ...
  const { error: confirmDbErr } = await admin.from('applications').update({
    ...
    stripe_payment_type: 'road_trip_circuit-mont-tremblant-2026',
  }).eq('email', normalEmail)
  if (confirmDbErr) captureException(confirmDbErr, { context: 'cmt-member-confirm-db', piId: pi.id })
  ...
  await markRegistrationPaid(admin, normalEmail, EVENT_NAME).catch(err => captureException(err, { context: 'cmt-member-confirm-mark-paid', piId: pi.id }))
  ...
  const checkinUrl = await getRouteCheckinUrl(admin, 'road_trip_circuit-mont-tremblant-2026', normalEmail).catch(() => null)
  ...
  after(() => Promise.allSettled([
    fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        text: `Hey ${firstName},\n\nYour payment of ${amount} for ${EVENT_NAME} is confirmed.\n\n${checkinUrl ? `Complete your trip details here: ${checkinUrl}\n\n` : ''}Meetup is at 12 PM in Montreal — full details closer to the date. Follow @canvasroutes on Instagram for updates.\n\nSee you on the road,\nJerry\nCanvas Routes`,
      }),
    }).then(r => { if (r && !r.ok) captureMessage(`Resend non-200 — cmt-member-confirm-member-email`, { status: r.status }) }).catch(err => captureException(err, { context: 'cmt-member-confirm-member-email', email: normalEmail })),
    fetch('https://api.resend.com/emails', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${process.env.RESEND_API_KEY}` },
      body: JSON.stringify({
    ...
    }).then(r => { if (r && !r.ok) captureMessage(`Resend non-200 — cmt-member-confirm-admin-email`, { status: r.status }) }).catch(err => captureException(err, { context: 'cmt-member-confirm-admin-email', email: normalEmail })),
  ]))
