...
export async function POST(request) {
  ...
  const { name, email, year, carMake, carModel, phone, passengers, hasChildren, childrenAges, more, source, dob, isMember, lang, _hp, _health_check } = body
  ...
  if (process.env.NEXT_PUBLIC_SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY) try {
    ...
    const newReg = {
      ...
      details: {
        ...
        phone: phone || null, passengers: passengers || null,
        has_children: hasChildren || null, children_ages: childrenAges || null,
