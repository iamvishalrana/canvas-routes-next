...
export async function GET() {
  // Returns member's existing Calabogie Boogie registration status
  try {
    ...
    captureException(e, { context: 'calabogie-member-register-get' })
...
export async function POST(request) {
  ...
  try {
    ...
    // calabogie-boogie-2026-register/route.js for why a loose prefix match
    // (just 'The Calabogie Boogie') would risk colliding with next year's edition.
  ...
  try {
    ...
    if (upsertErr) captureException(upsertErr, { context: 'calabogie-member-register-db', email: normalEmail })
    else if (appData?.id) {
      const { error: contactErr } = await admin.from('contacts').upsert(
        { application_id: appData.id },
        { onConflict: 'application_id', ignoreDuplicates: true }
      )
      if (contactErr) captureException(contactErr, { context: 'calabogie-member-register-contacts' })
    }
  } catch (e) {
    captureException(e, { context: 'calabogie-member-register-db-outer', email: normalEmail })
  }
  ...
  try {
    ...
    // Store PI ID immediately so calabogie-boogie-member-confirm can find this row after payment
