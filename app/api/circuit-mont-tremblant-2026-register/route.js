...
export async function POST(request) {
  ...
  try {
    ...
    // Match on the FULL event name (not just 'The Circuit Mont-Tremblant') — this
  ...
  if (process.env.NEXT_PUBLIC_SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY) try {
    ...
    if (upsertErr) {
      captureException(upsertErr, { context: 'cmt-register-db-upsert', email: normalEmail })
    } else if (appData?.id) {
      const { error: contactErr } = await supabase.from('contacts').upsert(
        { application_id: appData.id },
        { onConflict: 'application_id', ignoreDuplicates: true }
      )
      if (contactErr) captureException(contactErr, { context: 'cmt-register-contacts', email: normalEmail })
    }
  } catch (e) {
    captureException(e, { context: 'cmt-register-db', email: normalEmail })
  }
  ...
  try {
    ...
    if (process.env.NEXT_PUBLIC_SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY) {
      ...
      if (piStoreErr) captureException(piStoreErr, { context: 'cmt-register-pi-store', email: normalEmail })
    }
    ...
    if (process.env.RESEND_API_KEY && !_health_check) {
      after(() =>
        ...
        }).then(r => { if (r && !r.ok) captureMessage(`Resend non-200 — cmt-register-admin-notify`, { status: r.status }) }).catch(err => captureException(err, { context: 'cmt-register-admin-notify', email: normalEmail }))
      )
    }

    return Response.json({ clientSecret: pi.client_secret })
  } catch (err) {
    captureException(err, { context: 'cmt-create-payment-intent', email: normalEmail })
