...
// Jerry links upcoming_routes.event_id for slug 'rise-and-drive' to a
// generic check-in event.
export async function GET() {
  const admin = createAdminClient()

  const { data: route } = await admin.from('upcoming_routes').select('event_id').eq('slug', 'rise-and-drive').maybeSingle()
  ...
  try {
    ...
    captureException(err, { context: 'rad-roster' })
    return Response.json({ participants: [] })
  }
}
