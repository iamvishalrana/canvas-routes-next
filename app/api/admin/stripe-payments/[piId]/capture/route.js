...
export async function POST(request, { params }) {
  ...
  if (pi.metadata?.type?.startsWith('road_trip_') && pi.metadata?.event_name) {
    ...
    await markRegistrationPaid(supabase, payerEmail, pi.metadata.event_name).catch(err => captureException(err, { context: 'admin-capture-mark-paid', piId }))
  }
  ...
  if (process.env.RESEND_API_KEY && pi.metadata?.type?.startsWith('membership_')) {
    ...
    after(() => Promise.allSettled([
      ...
      }).then(r => { if (r && !r.ok) captureMessage(`Resend non-200 — admin-capture-membership-email`, { status: r.status }) }).catch(err => captureException(err, { context: 'admin-capture-membership-email', piId })),
    ]))
  }
  ...
  if (process.env.RESEND_API_KEY && pi.metadata?.type?.startsWith('road_trip_') && pi.metadata?.is_member !== 'yes') {
    ...
    after(() => Promise.allSettled([
      ...
      }).then(r => { if (r && !r.ok) captureMessage(`Resend non-200 — admin-capture-admin-email`, { status: r.status }) }).catch(err => captureException(err, { context: 'admin-capture-admin-email', piId })),
    ]))
  }
