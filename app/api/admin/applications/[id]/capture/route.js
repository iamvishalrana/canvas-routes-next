...
export async function POST(request, { params }) {
  ...
  try {
    ...
    if (process.env.RESEND_API_KEY && err.type === 'StripeCardError' && app.email) {
      ...
      after(() =>
        ...
        }).then(r => { if (r && !r.ok) captureMessage('Resend non-200 — admin-app-capture-declined-email', { status: r.status }) })
          .catch(e => captureException(e, { context: 'admin-app-capture-declined-email', appId: id }))
      )
    }
