...
export async function GET(request) {
  ...
  // Photo mode: full-bleed image + centered logo, no text (used by /routes)
  if (type === 'photo') {
    const bg = searchParams.get('bg') || '/convoy-hero.jpg'
    const bgUrl = bg.startsWith('http') ? bg : `${origin}${bg}`
    return new ImageResponse(
      (
        <div style={{ display: 'flex', width: '100%', height: '100%', position: 'relative' }}>
          <img
            src={bgUrl}
            style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center 45%' }}
          />
