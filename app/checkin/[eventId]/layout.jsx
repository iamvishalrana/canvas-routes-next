...
export async function generateMetadata({ params }) {
  ...
  const title = ev ? `Check In — ${ev.name}` : 'Event Check-In'
  const description = ev
    ? `Complete your check-in for ${ev.name} — trip details, waiver, and lunch selection.`
    : 'Complete your event check-in — trip details, waiver, and lunch selection.'

  const ogParams = new URLSearchParams({ type: 'event', title: 'Check In' })
  ...
  return {
    ...
    openGraph: {
      ...
      url: `https://canvasroutes.com/checkin/${eventId}`,
      title,
      description,
...
export default function CheckinEventLayout({ children }) {
  return children
}
